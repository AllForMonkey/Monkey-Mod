package com.afm.AfmMod.world.gen.generators;
...
public class WorldGenPalmTree extends WorldGenAbstractTree {
	...
	public static final IBlockState LOG = BlockInit.PALM_WOOD.getDefaultState();
	public static final IBlockState LEAF = BlockInit.PALM_LEAVES.getDefaultState();
	...
	public boolean generate(World world, Random rand, BlockPos pos) 
	{
		...
		else
		{
			...
			boolean isSoil = state.getBlock().canSustainPlant(state, world, down, net.minecraft.util.EnumFacing.UP, (IPlantable)Blocks.CACTUS);
						...
						for(int zPos = z - b2; zPos <= z + b2; zPos++)
						{
							...
							if(Math.abs(b3) != b2 || Math.abs(b4) != b2 || rand.nextInt(2) != 0 && b1 != 0)
							{
								...
								if(treeState.getBlock().isAir(treeState, world, treePos) || treeState.getBlock().isAir(treeState, world, treePos))
								{
									...
									this.setBlockAndNotifyAdequately(world, treePos.add(0, -5.0 * height, 5.0), LEAF);
									this.setBlockAndNotifyAdequately(world, treePos.add(0, -6.0 * height, 5.0), LEAF);
								}
							}
						}
					}
	...
	protected boolean canGrowInto(Block blockType)
	{
        ...
        return material == Material.AIR || material == Material.LEAVES || material == Material.GROUND || material == Material.SAND || blockType == Blocks.SAND || blockType == Blocks.GRASS || blockType == Blocks.DIRT || blockType == Blocks.LOG || blockType == Blocks.LOG2 || blockType == Blocks.SAPLING || blockType == Blocks.VINE;
